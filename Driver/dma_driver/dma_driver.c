...
MODULE_AUTHOR("FTN");
MODULE_DESCRIPTION("Test Driver for DMA controller IP.");
...
static ssize_t dma_read(struct file *f, char __user *buf, size_t len, loff_t *off);
static ssize_t dma_write(struct file *f, const char __user *buf, size_t length, loff_t *off);
...
int dma_init_x(void __iomem *base_address);
...
static struct dma_info *vp = NULL;
...
static int dma_probe(struct platform_device *pdev)
{
  ...
  printk(KERN_INFO "dma_probe: Probing\n");
  // Get phisical register adress space from device tree
  r_mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
  if (!r_mem)
  {
    printk(KERN_ALERT "dma_probe: Failed to get reg resource\n");
    return -ENODEV;
  }
  // Get memory for structure dma_info
  vp = (struct dma_info *)kmalloc(sizeof(struct dma_info), GFP_KERNEL);
  if (!vp)
  {
    printk(KERN_ALERT "dma_probe: Could not allocate memory for structure dma_info\n");
    return -ENOMEM;
  }
  ...
  {
    printk(KERN_ALERT "dma_probe: Could not lock memory region at %p\n", (void *)vp->mem_start);
    rc = -EBUSY;
    goto error1;
  }
  ...
  if (!vp->base_addr)
  {
    printk(KERN_ALERT "dma_probe: Could not allocate memory for remapping\n");
    rc = -EIO;
    goto error2;
  }
  ...
  if (!vp->irq_num)
  {
    printk(KERN_ERR "dma_probe: Could not get IRQ resource\n");
    rc = -ENODEV;
    goto error2;
  }
  ...
  {
    printk(KERN_INFO "dma_probe: Registered IRQ %d\n", vp->irq_num);
  }

  /* INIT DMA */
  dma_init_x(vp->base_addr);
  dma_simple_write(tx_phy_buffer, MAX_PKT_LEN, vp->base_addr); // helper function, defined later

  printk(KERN_NOTICE "dma_probe: DMA platform driver registered\n");
  return 0; //ALL OK
...
static int dma_remove(struct platform_device *pdev)
{
  ...
  printk(KERN_INFO "dma_probe: resseting");
  ...
  release_mem_region(vp->mem_start, vp->mem_end - vp->mem_start + 1);
  kfree(vp);
  printk(KERN_INFO "dma_probe: DMA removed");
  return 0;
}
...
static ssize_t dma_write(struct file *f, const char __user *buf, size_t length, loff_t *off)
{
  ...
  if (ret != -EINVAL) //checking for parsing error
  {
    if (xpos > 639)
    {
      printk(KERN_WARNING "DMA_write: X_axis position exceeded, maximum is 639 and minimum 0 \n");
    }
    else if (ypos > 479)
    {
      printk(KERN_WARNING "DMA_write: Y_axis position exceeded, maximum is 479 and minimum 0 \n");
    }
    else
    {
  ...
  {
    printk(KERN_WARNING "DMA_write: Wrong write format, expected \"xpos,ypos,rgb\"\n");
...
static int __init dma_init(void)
{
  ...
  printk(KERN_INFO "dma_init: Initialize Module \"%s\"\n", DEVICE_NAME);
  ret = alloc_chrdev_region(&my_dev_id, 0, 1, "DMA_region");
  if (ret)
  {
    printk(KERN_ALERT "dma_init: Failed CHRDEV!\n");
    return -1;
  }
  printk(KERN_INFO "dma_init: Successful CHRDEV!\n");
  my_class = class_create(THIS_MODULE, "DMA_drv");
  if (my_class == NULL)
  {
    printk(KERN_ALERT "dma_init: Failed class create!\n");
    goto fail_0;
  }
  printk(KERN_INFO "dma_init: Successful class chardev1 create!\n");
  my_device = device_create(my_class, NULL, MKDEV(MAJOR(my_dev_id), 0), NULL, "dma");
  if (my_device == NULL)
  {
    goto fail_1;
  }

  printk(KERN_INFO "dma_init: Device created\n");
  ...
  if (ret)
  {
    printk(KERN_ERR "dma_init: Failed to add cdev\n");
    goto fail_2;
  }
  printk(KERN_INFO "dma_init: Module init done\n");
...
static void __exit dma_exit(void)
{
  ...
  printk(KERN_INFO "dma_exit: Exit device module finished\"%s\".\n", DEVICE_NAME);
}
